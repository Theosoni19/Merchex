...
def band_listings(request,band_id):
    band = Band.objects.get(id=band_id)
    return render(request, 'listings/band_listings.html', {'band':band})
...
def band_update(request, band_id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            form.save()
            return redirect('band-detail', band.id)  # type: ignore
    else:
...
def listing_update(request, listing_id):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            form.save()
            return redirect('listing-detail', listing.id)  # type: ignore
    else:
